...
class MyPlotMainWindow(QtMainWindowLoader):
    ...
    def actionSelectAbaqusPath(self):
        selectedfile = str(QtGui.QFileDialog.getOpenFileName(self, "Select Abaqus ODB file (.odb)",'*.odb'))
    ...
    def actionAdd_Path_Coordinates(self):
        vector, ok = QtGui.QInputDialog.getText(self, 'Insert the path coordinates','Coordinates Format: (x0,y0,z0);(x1,y1,z1);...,')
        if ok:
            #------In here, the format of the path will be checked.----------------
            #Check if it is empty
            if vector =='':
                QtGui.QMessageBox.warning(self, "Error",'No coordinates were inserted.')
                return
                ...
                for i in np.arange(0,len(tempvector)): #Cycle to all the points inserted
                    ...
                    try:
                        icoordList=map(float,icoordinate.split(','))#Convert the string in a list of float numbers
                         ...
                         QtGui.QMessageBox.warning(self, "Error",'The vector inserted is not valid. Please check the format of the coordinates.')
                         return
    ...
    def actionSelectOutputPath(self):
        selectdir = str(QtGui.QFileDialog.getExistingDirectory(self, "Select Abaqus ODB file (.odb)"))
    ...
    def actionHelpStressStrainExtract(self):
        QtGui.QMessageBox.information(self, "Tool to extract the stress/strain along a pre-defined path",
    ...
    def actionHelpRotateAxisCoordinate(self):        
        QtGui.QMessageBox.information(self, "Help - Rotate Axis",
    ...
    def actionHelpPathCoordinates(self):
        QtGui.QMessageBox.information(self, "Help - Optical Fibre Path Coordinates",
    ...
    def actionHelpDisplacementStep(self):
        QtGui.QMessageBox.information(self, "Help - Time increment (Step)",
    ...
    def actionLoadFolderOSA(self):
        selectedfile = QtGui.QFileDialog.getOpenFileNames(self, "Select file(s) with stress and strain along the path (.txt).",'*.txt')
    ...
    def actionAddFBGPosition(self):
        ...
        for i in range(0,numberFBG):
            position, ok = QtGui.QInputDialog.getText(self, 'FBG:'+str(i+1)+' position','Insert the FBG:'+str(i+1)+' position. (mm or m from the begin of the line)')
            if ok:
                #------In here, the format of the path is checked.----------------
                #Check if it is empty
                if position =='':
                    QtGui.QMessageBox.warning(self, "Error",'No value inserted.')
                    return
                 #Try to convert to float
                else:
                    try:position=float(position)
                    except:
                        QtGui.QMessageBox.warning(self, "Error",'Invalid format.')
                        return
                    if self.FBGPosition:
                        if position <= max(self.FBGPosition):
                            QtGui.QMessageBox.warning(self, "Error",'The FBG position should be larger than the previous inserted.')
    ...
    def actionAddFBGOriginalWavelength(self):
        ...
        numberFBG=self.ui.spinBox_NumberFBG.value()
        for i in range(0,numberFBG):
            wavelength, ok = QtGui.QInputDialog.getText(self, 'FBG:'+str(i+1)+' original wavelength','Insert the FBG:'+str(i+1)+' original wavelength. (in nm)')
            if ok:
                #------In here, the format of the path is checked.----------------
                #Check if it is empty
                if wavelength =='':
                 ...
                 #Try to convert to float
                    ...
                    if self.FBGOriginalWavel:
                        if wavelength <= max(self.FBGOriginalWavel):
                            QtGui.QMessageBox.warning(self, "Error",'The FBG original wavelength should be larger than the previous inserted.')
    ...
    def actionSaveAsFileOSA(self):
        ...
        if fsize:
            saveFile = str(QtGui.QFileDialog.getSaveFileName(self, "Save FBG Spectrum Plot as a file.",'*.txt'))
            if saveFile!='':
    ...
    def actionAboutFBGSpectrumSimulation(self):
        QtGui.QMessageBox.information(self, "About FBG Spectrum Simulation (Specific Step)",
    ...
    def actionHelpFileFormatOSA(self):
        QtGui.QMessageBox.information(self, "Help - File Format OSA",
    ...
    def actionHelpFBGPositionOSA(self):
        QtGui.QMessageBox.information(self, "Help - FBG Position (mm or m)",
    ...
    def actionHelpFBGWavelengthOSA(self):
        QtGui.QMessageBox.information(self, "Help - FBG Original Wavelength OSA",
    ...
    def actionHelpTypeSimulationOSA(self):
        QtGui.QMessageBox.information(self, "Help - Type of Simulation OSA",
...
#-------------------Fibre Bragg Grating Array Configuration-------------------
    ...
    def actionSaveAsFileTR(self):
        ...
        if fsize:
            saveFile = str(QtGui.QFileDialog.getSaveFileName(self, "Save FBG time response as a file.",'*.txt'))
            if saveFile!='':
                savefile = open(saveFile,"w")
                #Write the Head
    ...
    def actionAboutTimeResponse(self):
        QtGui.QMessageBox.information(self, "About FBG Signal Variation (Time Response)",
...
class PlotWindowTR(QtDialogLoader):
    def __init__(self, parent, modal,TR):
       ...
       #Set the Axes names        
        ...
        #Plot the TR
        self.TRplot.axes0.hold(True)
        self.TRplot.axes1.hold(True)
        self.Plot()
    ...
    def actionSavePicture(self):
        savePictureFile = str(QtGui.QFileDialog.getSaveFileName(self, "Save FBG Spectrum Plot",'*.png'))
        if savePictureFile!='':
...
class TRCanvas(Canvas):
    ...
    def __init__(self, parent=None, title='', xlabel='', ylabel='',
        ...
        self.axes0.hold(hold)
        self.axes1.hold(hold)
